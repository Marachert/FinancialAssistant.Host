...
namespace FinancialAssistant.Expense.Api.Security;

public sealed class ExpenseGatewayAuthenticator
{
    public const string SharedSecretConfigurationKey = "Expense:Gateway:SharedSecret";

    private readonly byte[] sharedSecretHash;

    public ExpenseGatewayAuthenticator(IConfiguration configuration)
    {
    ...
    {
        var values = httpContext.Request.Headers[ExpenseGatewayHeaders.Authentication];
        if (values.Count != 1)
        {
            return false;
        }
