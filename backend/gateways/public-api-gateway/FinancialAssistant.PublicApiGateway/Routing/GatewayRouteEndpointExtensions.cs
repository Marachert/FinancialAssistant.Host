...
public static class GatewayRouteEndpointExtensions
{
    ...
    public static WebApplication MapGatewayRouteMap(this WebApplication app)
    {
        ...
        foreach (var route in routeCatalog.Routes)
        {
            ...
            app.MapMethods(route.PublicPattern, methods, (HttpContext context, GatewaySecurityBoundary securityBoundary, GatewayRequestDispatcher dispatcher) => HandleGatewayRouteAsync(context, securityBoundary, dispatcher, route));

            if (!string.IsNullOrWhiteSpace(route.CatchAllPattern))
            {
                app.MapMethods(route.CatchAllPattern, methods, (HttpContext context, GatewaySecurityBoundary securityBoundary, GatewayRequestDispatcher dispatcher) => HandleGatewayRouteAsync(context, securityBoundary, dispatcher, route));
            }
        }
